...
func (repo *DbUserRepo) Store(user usecases.User) {
	...
	repo.dbHandler.Execute(
		fmt.Sprintf(
			"INSERT INTO users (id, customer_id, is_admin) VALUES ('%d', '%d', '%v')",
			user.Id,
			user.Customer.Id,
			isAdmin,
		),
...
func (repo *DbUserRepo) FindById(id int) usecases.User {
	row := repo.dbHandler.Query(
		fmt.Sprintf("SELECT is_admin, customer_id FROM users WHERE id = '%d' LIMIT 1", id),
	)
	var isAdmin string
	var customerId int
	row.Next()
...
func (repo *DbCustomerRepo) Store(customer domain.Customer) {
	repo.dbHandler.Execute(
		fmt.Sprintf(
			"INSERT INTO customers (id, name) VALUES ('%d', '%v')",
			customer.Id,
			customer.Name,
		),
	)
...
func (repo *DbCustomerRepo) FindById(id int) domain.Customer {
	row := repo.dbHandler.Query(
		fmt.Sprintf("SELECT name FROM customers WHERE id = '%d' LIMIT 1", id),
	)
	var name string
	row.Next()
	row.Scan(&name)
...
func (repo *DbOrderRepo) Store(order domain.Order) {
	repo.dbHandler.Execute(
		fmt.Sprintf(
			"INSERT INTO orders (id, customer_id) VALUES ('%d', '%v')",
			order.Id,
			order.Customer.Id,
		),
	)
	for _, item := range order.Items {
		repo.dbHandler.Execute(
			fmt.Sprintf(
				"INSERT INTO items2orders (item_id, order_id) VALUES ('%d', '%d')",
				item.Id,
				order.Id,
			),
		)
...
func (repo *DbOrderRepo) FindById(id int) domain.Order {
	row := repo.dbHandler.Query(
		fmt.Sprintf("SELECT customer_id FROM orders WHERE id = '%d' LIMIT 1", id),
	)
	var customerId int
	row.Next()
	row.Scan(&customerId)
	...
	row = repo.dbHandler.Query(
		fmt.Sprintf("SELECT item_id FROM items2orders WHERE order_id = '%d'", order.Id),
	)
	for row.Next() {
		row.Scan(&itemId)
...
func (repo *DbItemRepo) Store(item domain.Item) {
	...
	repo.dbHandler.Execute(
		fmt.Sprintf(
			"INSERT INTO items (id, name, value, available) VALUES ('%d', '%v', '%f', '%v')",
			item.Id,
			item.Name,
			item.Value,
			available,
...
func (repo *DbItemRepo) FindById(id int) domain.Item {
	row := repo.dbHandler.Query(
		fmt.Sprintf("SELECT name, value, available FROM items WHERE id = '%d' LIMIT 1", id),
	)
	var name string
	var value float64
